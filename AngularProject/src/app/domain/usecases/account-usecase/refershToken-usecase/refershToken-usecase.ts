...
export class RefershTokenAccountUseCase implements 
    UseCase<Param<{ AccessToken: string, RefreshToken: string }>, ApiResponseModel<AccountEntity>> {

    constructor(private iRepository: IAccountRepository) { }
    execute(params: Param<{ AccessToken: string, RefreshToken: string }>): Observable<ApiResponseModel<AccountEntity>> {
        return this.iRepository.RefreshToken(params.payload);
    }
